...
- 用户手动测试确认此前记录的旧回归问题均已清除。
- 本文原 10 项待办全部关闭；后续新问题不再沿用旧编号。
- 新发现的对白后半跳过、道具/菜单说明首空页问题转入 v22 单独排查。

## 2026-06-05 v23 新问题处理记录

- v21 已由用户手动确认清空旧 10 项回归 todo；v22 后新增的问题不再沿用旧编号。
- v23 针对 6 类新增问题生成候选 `rom/narutorpg3_chs_patcher_v23_menu_dialog_desc_fusion12_r2.nds`。
- 已静态修复：`zh_txt_6b929156_0001FA_0006`、`zh_txt_6b929156_0001A6_0005` 和上一轮 `zh_txt_2ef7aa25_000758_0027` 的控制位分段写回；道具/忍术说明 NUL4 首空页；成员/Wi-Fi/菜单多段说明子槽位；装备/状态页左侧固定宽度字段；成员设置运行时 overlay 副本；Wi-Fi 好友菜单剩余未翻译文本。
- 静态验证：文本 5859 条、菜单 289 条实际写回字节差异均为 0；Header CRC OK / Banner CRC OK；截图相关 CP932 日文关键字除下载包 `data/download/n3dl.srl` 内嵌 `忍術` 外均为 0 命中。
- 本轮未使用 DeSmuME/MCP；运行时显示效果等待用户手动验证。

## 2026-06-05 v24 新问题处理记录

- 旧 v5/v20 回归 todo 已在 v21 由用户手动确认清零；本轮继续处理 v23 后新增问题，不重新打开旧编号。
- v24 候选：`rom/narutorpg3_chs_patcher_v24_save_equip_trim_fusion12.nds`。
- 本轮修复三类静态根因：
  - overlay_0002 保存确认选项必须按原始可见宽度填满，避免“是”后 `00 00` 提前终止导致“否”不显示。
  - 固定 `CTRL_0000` 子槽位译文后改为 `00` 填充；`msg/menu/*` 与 `msg/wifi/friend_msg.msg` 普通 `03 00` 菜单说明改为译文后立即结束，清除空白说明页和 `@` 占位符风险。
