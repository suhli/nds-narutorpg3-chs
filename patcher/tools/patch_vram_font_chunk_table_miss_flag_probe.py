...
MISS_FLAG_ADDR = split.CURRENT_CHAR_ADDR + 0x04
MISS_CHAR_ADDR = split.CURRENT_CHAR_ADDR + 0x08
MISS_CHUNK_ID_ADDR = split.CURRENT_CHAR_ADDR + 0x0C
MISS_MODE_ADDR = split.CURRENT_CHAR_ADDR + 0x10
MISS_VARS_END_ADDR = MISS_MODE_ADDR + 0x04
...
def build_payload() -> bytes:
    ...
    if MISS_VARS_END_ADDR > split.SAVE_CHAR_HOOK_ADDR + split.PATCH_SIZE:
        raise ValueError("miss vars exceed payload")
    ...
    payload[vars_off : MISS_VARS_END_ADDR - split.SAVE_CHAR_HOOK_ADDR] = bytes(
        MISS_VARS_END_ADDR - split.CHS_1X1_MAP_PTR_ADDR
    )
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Build a miss-flag chunk-table probe ROM.")
    parser.add_argument("--work", default="rom/unpacked/vram_font_chunk_table_miss_flag_probe")
    parser.add_argument("--output", default="rom/test_vram_font_chunk_table_miss_flag_probe.nds")
    ...
    slim.patch_overlay(work / "overlay" / "overlay_0000.bin")
    ...
    print(f"miss_flag=0x{MISS_FLAG_ADDR:08X}")
    print(f"miss_char=0x{MISS_CHAR_ADDR:08X}")
    print(f"miss_chunk_id=0x{MISS_CHUNK_ID_ADDR:08X}")
    print(f"miss_mode=0x{MISS_MODE_ADDR:08X}")
